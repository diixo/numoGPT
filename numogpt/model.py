...
class CausalSelfAttention(nn.Module):
    ...
    def __init__(self, config):
        ...
        #TODO: self.dropout = config.dropout
    ...
    def __init__(self, config):
        ...
        if type_given:
            ...
            config.merge_from_dict({
                ...
                'gpt-numo':     dict(n_layer=4, n_head=4, n_embd=64),
        ...
        for mn, m in self.named_modules():
            for pn, p in m.named_parameters():
                ...
                if pn.endswith('bias'):
                    ...
                    no_decay.add(fpn)
                elif 'attn.U' in fpn or 'attn.V' in fpn:
                    no_decay.add(fpn)
...
"""
def adjust_probabilities_with_semantics(probs, idx, semantic_model):
    # Получаем семантическую оценку текущего контекста
    context_embedding = get_context_embedding(idx)  # например, через BERT, GPT или другие модели
    semantic_scores = semantic_model.predict(context_embedding)

    # Снижаем вероятность для токенов, которые не соответствуют семантическому контексту
    for token_idx in range(probs.size(-1)):
        if not is_semantically_appropriate(token_idx, semantic_scores):
            probs[:, token_idx] = 0  # Обнуляем вероятность для неподобающих токенов
